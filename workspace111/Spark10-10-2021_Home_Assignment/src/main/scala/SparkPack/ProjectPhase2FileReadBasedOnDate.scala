...
object ProjectPhase2FileReadBasedOnDate {
	def main(args:Array[String]):Unit={
			val conf = new SparkConf().setAppName("spark").setMaster("local[*]")
					...
					//			    val df = spark.read.format("com.databricks.spark.avro").load("file:/user//cloudera//data/projectsample.avro")
					val data = spark.read.format("com.databricks.spark.avro").load(s"file:///C://data//projdata//$yesterday_date/projectsample.avro")
					//			    df.show()
					//			    df.printSchema()     
					val filepath = s"file:///C://data//projdata//$yesterday_date/projectsample.avro"
					println("File picked up is: " + filepath)
					data.show()
