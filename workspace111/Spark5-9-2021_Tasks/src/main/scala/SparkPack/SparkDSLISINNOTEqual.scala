...
object SparkDSLISINNOTEqual {
  def main(args:Array[String]):Unit={
			val conf = new SparkConf().setAppName("Spark").setMaster("local[*]")  // Vice President
					...
					println("=================Filter data with DSL ISIN & NOT EQUAL Condition============")
					val filterdf =  df.filter(col("category").isin("Gymnastics","Team Sports") && col("spendby")=!="")
